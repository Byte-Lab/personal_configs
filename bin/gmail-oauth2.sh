...
refresh_token="${MAIL_ACCESS_TOKEN}"

while (( "$#" )); do
  case "$1" in
    --client-id)
      echo "ERROR: CLIENT ID hard coded into script"
      exit
      ;;
    --client-secret)
      echo "ERROR: SECRET hard coded into script using gpg"
      exit
      ;;
    --refresh-token)
      echo "ERROR: TOKEN hard coded into script gpg"
      shift 2
      ;;
    *)
      args="${args} ${1}"
...
client_secret="$(gpg --batch -q --decrypt -r $GPG_USER_ID ~/.local/gpg/oauth_secret.gpg)"
